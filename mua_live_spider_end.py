    ...
    def crawl(self):
        ...
        a = self.wait.until(
            EC.element_to_be_clickable(
                (By.ID, 'cn.dps05.live:id/tv_count_down')))
        a.click()
        # 点击关闭广告
        b = self.wait.until(
            EC.element_to_be_clickable(
                (By.ID, 'cn.dps05.live:id/btn_know')))
        b.click()
        # 点击主播页面
        try:
...
'''
获取元素坐标点
imgLocation=click_copy.location
imgSize = click_copy.size
rangle = (int(imgLocation['x']), int(imgLocation['y']), int(imgSize['width']), int(imgSize['height']))
location=rangle
left = location[0]
top = location[1]
right = location[2] + left
bottom = location[3] + top
print(left)
print(top)
print(right)
print(bottom)
